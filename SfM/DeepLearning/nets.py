...
def pose_exp_net(tgt_image, src_image_stack, do_exp=True, is_training=True):
    ...
    with tf.variable_scope('pose_exp_net') as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            # cnv1_1 = slim.conv2d(inputs,16,  [3, 3], stride=2, scope='cnv1')
            # cnv1_2  = slim.conv2d(cnv1_1,32,  [3, 3], stride=2, scope='cnv1')
            # cnv1_3  = slim.conv2d(cnv1_2,64,  [3, 3], stride=2, scope='cnv1')
            # cnv2  = slim.conv2d(cnv1_3, 64,  [3, 3], stride=2, scope='cnv2')
...
def disp_net(tgt_image, is_training=True):
    ...
    # tgt_image = tf.concat([tgt_image, src_image_stack], axis=3)
    # H = tgt_image.get_shape()[1].value
    # W = tgt_image.get_shape()[2].value
    # num_source = int(src_image_stack.get_shape()[3].value//3)
