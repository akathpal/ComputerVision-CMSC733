...
class SfMLearner(object):
    ...
    def var_learning_rate(self,curr_step,learning_rate,step_inc):
        if(curr_step%step_inc == 0):
            learning_rate *= 0.8
    ...
    def SSIM(self, x, y): 
        ...
        # mods
        tmp = tf.clip_by_value((1 - SSIM) / 2, 0, 1)
        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    ...
                    # SSIM loss
                    ssim_loss += opt.ssim_weight * self.SSIM(curr_proj_image, curr_tgt_image)
    ...
    def train(self, opt):
        ...
        with sv.managed_session(config=config) as sess:
            ...
            for step in range(1, opt.max_steps):
                ...
                # Change Learning rate. Last argument is the number of steps after
                # which it should be updated
                opt.learning_rate = self.var_learning_rate(step,opt.learning_rate,opt.step_lr)
