...
class PRN_two:
    ...
    def __init__(self, is_dlib = False, prefix = '.'):
        ...
        if is_dlib:
            import dlib
            detector_path = os.path.join(prefix, 'prnet/Data/net-data/mmod_human_face_detector.dat')
            self.face_detector = dlib.cnn_face_detection_model_v1(
                    detector_path)

        #---- load PRN 
        self.pos_predictor = PosPrediction(self.resolution_inp, self.resolution_op)
        prn_path = os.path.join(prefix, 'prnet/Data/net-data/256_256_resfcn256_weight')
        ...
        # uv file
        self.uv_kpt_ind = np.loadtxt(prefix + '/prnet/Data/uv-data/uv_kpt_ind.txt').astype(np.int32) # 2 x 68 get kpt
        self.face_ind = np.loadtxt(prefix + '/prnet/Data/uv-data/face_ind.txt').astype(np.int32) # get valid vertices in the pos map
        self.triangles = np.loadtxt(prefix + '/prnet/Data/uv-data/triangles.txt').astype(np.int32) # ntri x 3
    ...
    def process(self, input, image_info = None):
        ...
        if image_info is not None:
            ...
            if len(detected_faces) != 2:
                print('warning: 2 faces not detected')
                return None

            print("{} faces detected".format(len(detected_faces)))
            final_pos = []

            for i in xrange(len(detected_faces)):
                d = detected_faces[i].rect 
                ...
                t = image/255.
                cropped_image = warp(t, tform.inverse, output_shape=(self.resolution_inp, self.resolution_inp))
